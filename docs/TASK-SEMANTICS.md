...
- **默认模板往返格式**（可被 TF-018 parser 重新导入）：Front Matter（title/generated_at）+ `##`/`###` 标题层级 + `- 状态: <key>` 元数据行 + 优先级/标签/负责人行 + `依赖: <Markdown 锚点链接>`（TF-039/040：依赖导出为 `[标题](#锚点)`——可读、可点击跳转文档内标题；parser 解析时提取链接文本按标题匹配，往返一致）。

### 18.2 导出目标与结果

- `target: overwrite`：path 必填（覆盖指定文件）；`target: copy`：path 缺省 `{workdir}/.taskboard/export-<时间戳>.md`（TF-039：每次导出独立文件，避免覆盖上一次结果）。
