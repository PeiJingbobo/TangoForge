...
> **版本**：v1.1（2026-08-12，新增 P7 知识库 TF-044~TF-052）
...
| P5.7 项目右键菜单   | 1      | 1      | 0      | 0      |
| P5.8 PERT 任务图    | 1      | 1      | 0      | 0      |
| P6.0 发布与在线更新 | 1      | 0      | 1      | 0      |
| P6 测试与交付       | 3      | 0      | 0      | 3      |
| P7 知识库           | 9      | 9      | 0      | 0      |
| P8 守护进程运维     | 1      | 1      | 0      | 0      |
| **合计**            | **47** | **44** | **1**  | **2**  |
...
## P5.8 PERT 任务图（TF-055，用户需求）

### TF-055 PERT 图替换全景图（P0，独立）🟡 带核验

- **涉及模块**：`app/src/lib/pert-layout.ts`、`app/src/components/graph/pert-view.tsx`、`app/src/components/graph/pert-legend.tsx`、`app/src/components/graph/graph-view-toggle.tsx`、`app/src/hooks/useGraphViewMode.ts`、`app/src/features/tasks/GraphPage.tsx`
- **描述**：2026-08-18 按用户反馈重构 PERT 视图（后端 Go 零 diff，复用 `GET /api/graph` 契约）：
  - **布局**：最长路径分层 + 32 轮同层 barycenter，圆形节点固定行网格；
  - **布线**：仅消费 `dependency` 边，同源边共享右侧端点与短主干，长边沿节点行间走廊正交布线；
  - **路径选择**：16px 透明命中层；选边后高亮两端、源端全部上游、目标端全部下游，旁路降噪；
  - **画布**：固定 SVG 视口，空白处任意拖拽平移、滚轮缩放、真实 bounds 适应屏幕；
  - **回归**：保留搜索、状态色、节点单击/双击和力导向切换。
- **验收标准**：
  - [x] PERT 定向测试 36 例全绿（Graphviz B 样条/共享端点/节点避让/溯源/可达/选边/平移缩放绑定）
  - [x] 前端 303 例测试全绿 + lint/typecheck/format/electron build 通过
  - [ ] 真实 Electron GUI 人工视觉、拖拽手感与复杂路径可读性核验
  - [x] 后端 Go 代码零 diff（HTTP /api/graph / MCP graph_get / CLI 行为不变）
- **方案文件**：`docs/FEATURES/pert-graph.md`
- **总结文件**：`docs/record/TF-055-PERT任务图重构-部分完成.md`
- **人工验证**：`docs/record/TF-055-PERT任务图重构-人工验证手册.md`
- **任务板说明**：TangoForge 实时任务板未登记 TF-055，本次沿用 TF-028 跟踪，交付时流转 `verifying`。

---

## P6.0 发布与在线更新（TF-036，用户需求）

### TF-036 CI/CD 自动发布 + 在线更新（P0，依赖 docs/CI-CD-UPDATER.md 评审通过）🟠 带核验

- **涉及模块**：`.github/workflows/release.yml`、`app/electron-builder.yml`、`app/electron/updater.ts`、`app/electron/preload.ts`、`app/electron/main.ts`、`app/src/features/settings/UpdateSection.tsx`、`app/src/types/update.ts`、`README.md`、`CHANGELOG.md`
- **描述**：按 `docs/CI-CD-UPDATER.md`（v1.0 已评审）落地：
  - **release.yml**：`v*` 标签推送触发（`workflow_dispatch` 兜底）→ tag==`app/package.json` version 强一致校验 → 质量门禁（前端 typecheck/lint/format/test + 后端 vet/test/覆盖率）→ 创建 GitHub Release 骨架 → `macos-14`（arm64）未签名打包（dmg+zip+latest-mac.yml）+ `windows-latest`（x64）自签名打包（nsis+portable+latest.yml）→ `--publish always` 上传 Release；
  - **在线更新**：`electron-updater` 接入主进程/IPC/preload（Windows 全链路：检测→下载→重启安装）；macOS 未签名阶段「检测新版本 → 自动打开 dmg 下载页 → 用户手动安装」（GitHub Releases API，每个版本仅自动打开一次）；
  - **UI**：「设置 → 关于」tab（当前版本 + 检查更新 + 状态/下载/安装），启动后延迟 10s 后台自动检查；
  - **版本**：`app/package.json` 0.1.0 → 0.5.0 → 0.6.0（v0.5.0 打包链路失败，首个 CI 发布为 `v0.6.0`）；README 增「下载与更新」+ macOS 未签名允许运行指令；新增 `CHANGELOG.md`。
- **验收标准**：
  - [x] 本地 typecheck / lint / format / test（241 用例）/ coverage 全绿
  - [x] electron-builder 本机 mac 出包冒烟（app-update.yml + latest-mac.yml + electron-updater 打包成功）
  - [ ] GitHub Actions release.yml 实跑通过（mac/win 双产物 + latest*.yml 上传 Release）
  - [ ] Windows 旧版安装 → 推送新版本 → App 提示更新 → 下载 → 重启安装成功
  - [ ] macOS 旧版安装 → 推送新版本 → App 自动打开 dmg 下载页
- **总结文件**：`docs/record/TF-036-CI-CD自动发布与在线更新-*.md`
...
> 方案依据：`docs/KNOWLEDGE-BASE.md`（v1.0，两轮 QA-K1~K22 评审通过）。核心：知识库 = 文档引用注册表 + 任务关联 + 语义索引（摘要/向量），**不存原文**；命名多库、文档按 `abs_path` 唯一 + 单向量；LLM 导入草稿流扩展 `knowledge_files`；fsnotify + 防抖自动重索引；纯 Go 余弦检索。

### TF-044 知识库数据模型与迁移 v4（P0，依赖 TF-001）

- **涉及模块**：`internal/db`、`internal/knowledge`
- **描述**：按 `docs/KNOWLEDGE-BASE.md §3` 实现迁移（实际落地 **v5**，规划 v4 已被 TF-040 number 占用）：新建 5 表（`knowledge_bases / knowledge_documents / knowledge_base_documents / task_documents / knowledge_chunks`）及索引；`project.Import` 初始化时自动创建「默认库」（`is_default=1`）；向量存储 f32 LE BLOB + `dim` + `embedding_model`。
- **验收标准**：
  - [x] `go test ./internal/db/...` 全绿（`sqlite:memory:` 隔离）；迁移幂等、up/down 双向可用
  - [x] 断言 5 表结构/索引/唯一约束（`idx_kd_abs`、`UNIQUE(project_id, name)`、`UNIQUE(document_id, seq)`）
  - [x] 初始化项目后默认库存在；`CGO_ENABLED=0 go build ./...` 通过
- **产出文件**：`internal/db/migrate.go`（v5）、`internal/knowledge/`（常量定义）、`internal/project/project.go`（默认库初始化）
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-044-知识库数据模型与迁移v5-成功.md`

### TF-045 库/文档 CRUD 业务层（P0，依赖 TF-044）

- **涉及模块**：`internal/knowledge`、`internal/task`（只读校验）、`internal/config`
- **描述**：store + doclink：库 CRUD（重名拒绝、默认库不可转移）；文档注册（按 `abs_path` 唯一复用，文本/二进制类型判定）；**拷贝语义**（`copy: none/copy/auto`，外部文件拷贝进默认文档目录：项目配置覆盖 → `.taskboard/knowledge/`，记录 `origin_path`）；relink（更新路径 + history 追加 + 重置重建索引 + 清除 missing，保留库成员与任务关联）；解除引用（删文档 + chunks + 边）；任务关联/解除（`task_documents`）；**任务归档/删除保留关联**。
- **验收标准**：
  - [x] 单测覆盖：abs_path 唯一性、auto/copy/none 拷贝语义（含项目级默认目录配置）、relink 重建与 history、库删除仅移除库↔文档边、任务归档后关联保留、二进制仅注册
  - [x] 错误码正确（`KNOWLEDGE_NOT_FOUND/INVALID`、`DOCUMENT_NOT_FOUND/INVALID/MISSING`、`COPY_FAILED`）
  - [x] `internal/knowledge` 覆盖率 ≥ 90%（实测 91.2%）
- **产出文件**：`internal/knowledge/{knowledge,store,doclink,models,errors}.go`、`*_test.go`（7 个）、`internal/config/config.go`（Knowledge 节）
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-045-库文档CRUD业务层-成功.md`

### TF-046 LLM embedding 扩展 + 摘要生成（P1，依赖 TF-015）

- **涉及模块**：`internal/llm`、`internal/config`、`internal/knowledge`
- **描述**：`llm.Embedding`（openai 兼容 `POST {base}/embeddings` + ollama `POST {base}/api/embed` 双协议）；全局配置 `llm.embedding` 节（base_url 空复用 chat、api_key 回退、api_kind、timeout）；文档摘要（LLM CompleteJSON ≤200 字，缓存 key=`content_hash`，hash 失效重生成；摘要失败不阻断嵌入）；新增 `KnowledgeGlobalConfig` 全局 `knowledge` 节（QA-K18 默认 + QA-K23 向量开关 `*bool`）。
- **验收标准**：
  - [x] mock server 单测：双协议请求构造与响应解析、未配置 → `EMBEDDING_NOT_CONFIGURED`、调用失败 → `EMBEDDING_FAILED`
  - [x] 摘要缓存 hash 失效重生成；摘要失败文档仍可嵌入（`summary=""`）
  - [x] 不依赖厂商 SDK，可指向 Ollama 与 OpenAI 兼容端点
- **产出文件**：`internal/llm/embedding.go`(+test)、`internal/config/config.go`/`global.go`（embedding + knowledge 节）、`internal/knowledge/indexer.go`（摘要）(+test)、`internal/knowledge/store.go`（Options.LLM）
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-046-LLM-embedding扩展与摘要生成-成功.md`

### TF-047 分块 + 向量索引 + 余弦检索（P1，依赖 TF-045、TF-046）

- **涉及模块**：`internal/knowledge`
- **描述**：indexer：Markdown 标题分块（一标题一段，heading 记录）+ 无标题/超长按字符切分（≤800 + 100 重叠）；逐 chunk 嵌入写 `knowledge_chunks`；**纯 Go 余弦相似度**检索（文档得分 = 命中 chunk 最大相似度）返回 topK 文档 + 命中片段（每文档 ≤3）；`kb_id` 过滤、`threshold`（默认 0.3）；检索前懒校验 missing 标记；嵌入失败 → `status=failed+index_error` 可重试；`max_index_size` 超限仅注册+摘要。
- **验收标准**：
  - [x] 单测：标题分块/大小兜底边界、检索 topK/阈值/kb 过滤/命中片段、missing 文档标记、二进制不嵌入
  - [x] 检索为纯 Go 实现（无 CGO 依赖）；`CGO_ENABLED=0` 编译通过
- **产出文件**：`internal/knowledge/indexer.go`（IndexDocument）、`chunk.go`、`vector.go`、`search.go`、`*_test.go`
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-047-分块向量索引与余弦检索-成功.md`

### TF-048 文件扫描与防抖（P1，依赖 TF-047）

- **涉及模块**：`internal/knowledge`、`internal/config`、`cmd/daemon`
- **描述**：scanner：启动扫描（`startup_scan`）；fsnotify 监听已注册文档唯一父目录（可开关）；事件按文档合并到 `debounce_ms`（默认 30000）防抖窗口；全局 singleflight + `embed_concurrency=1` 串行索引；变化检测（mtime+size 快速比对 → sha256 确认）；`max_index_size`（512KB）超限仅注册+摘要不嵌入；`embedding_model` 漂移检测自动全量重嵌；失败 → `status=failed+index_error` 自动重试；`ContentHash==""` 首次索引强制补齐。
- **验收标准**：
  - [x] 单测（临时目录 + 注入事件）：防抖合并（窗口内多次事件一次索引）、启动扫描、手动扫描、懒校验、模型漂移重嵌、超限不嵌入、失败重试
  - [x] daemon 启动接入 scanner（enabled 开关关闭时零开销）
- **产出文件**：`internal/knowledge/scanner.go`(+test ×2)、`internal/config/global.go`（knowledge 节）
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-048-文件扫描与防抖-成功.md`

### TF-049 导入草稿流 knowledge_files + 导出资料行（P1，依赖 TF-045、TF-047、TF-018、TF-019）

- **涉及模块**：`internal/parser`、`internal/exporter`、`internal/knowledge`、`internal/api`
- **描述**：`ParseInput` 增 `knowledge` 节（file_paths/directory/kb_id/copy）；候选文件树 + 摘要缓存注入 prompt（结构 + 摘要，不拼全文）；LLM 输出 `knowledge_files:[{path, kb, reason}]`（path 必填，kb 不存在 → 整次失败）；confirm 时逐条注册/复用 + 拷贝 + 库归属 + 建立任务关联 + 触发异步索引，**路径缺失仅警告跳过**（dropped_knowledge 计数）；导出默认模板增 `- 资料: <path>` 行（有关联时输出），parser 回读「资料:」行重建关联。
- **验收标准**：
  - [x] 单测（mock LLM）：knowledge_files 解析/校验（缺 path、kb 不存在整次失败）、confirm 关联 + dropped 计数、copy 语义生效
  - [x] 导出含资料行且可被 parser 回读重建关联（往返一致）
- **产出文件**：`internal/parser/parser.go`（ParseInput/rawParseOutput/Confirm 扩展）、`internal/exporter/templates/default.tmpl`、`internal/parser/parser_test.go`、`internal/exporter/exporter_test.go`、`internal/knowledge/doclink.go`（LinkFiles）
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-049-导入草稿流knowledge-files与导出资料行-成功.md`

### TF-050 knowledge HTTP 端点 + WS + 审计 + 权限（P1，依赖 TF-045~~TF-049、TF-011~~TF-014）

- **涉及模块**：`internal/api`、`internal/auth`、`internal/task`（详情内嵌）、`internal/project`、`cmd/daemon`
- **描述**：`/api/knowledge/*` 端点（§6 表：bases CRUD、documents 注册/详情/阅读/编辑/relink/解除、link/unlink、search、scan、tasks/:id 关联列表）；`/api/tasks/:id` 内嵌 `knowledge_documents` 摘要；权限 `knowledge.read`（默认只读第 6 项）/`knowledge.write`/`knowledge.index` 接入；审计 `knowledge.*` 写操作；WS 事件 `knowledge.*`（document_added/updated/removed/relinked、task_linked/unlinked、kb_created/updated/deleted、scan_completed、index_failed）；daemon 启动接线 Scanner（注册全部项目 + 扫描/监听）。
- **验收标准**：
  - [x] 端点单测 + 集成冒烟（注册→关联→检索→编辑→relink→解除 全链路）
  - [x] denied 审计（knowledge.write/index 默认 false）；WS `knowledge.*` 事件可达；`/api/tasks/:id` 内嵌文档
- **产出文件**：`internal/api/handlers_knowledge.go`、`handlers_knowledge_test.go`、`internal/api/server.go`、`internal/api/errors.go`、`internal/project/project.go`、`cmd/daemon/main.go`
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-050-knowledge-HTTP端点WS审计权限-成功.md`

### TF-051 knowledge MCP 工具 + CLI（P2，依赖 TF-050）

- **涉及模块**：`internal/mcp`、`cmd/cli`
- **描述**：MCP 新增 8 工具（首参 `project`）：`knowledge_list / knowledge_search / knowledge_read / knowledge_link / knowledge_unlink / knowledge_relink / knowledge_scan / knowledge_edit`；权限映射同 HTTP；CLI 子命令 `tangoforge knowledge bases|documents|search|read|link|unlink|relink|scan|edit`（--project 强制，HTTP 等价）。
- **验收标准**：
  - [x] stdio/HTTP 双传输集成测试：knowledge 全生命周期 + denied 路径（write/index 默认拒绝）
  - [x] CLI 与 HTTP 结果一致（同一守护进程冒烟）
- **产出文件**：`internal/mcp/tools_knowledge.go`(+test)、`internal/mcp/mcp.go`、`cmd/cli/cmd_knowledge.go`、`cmd/cli/cmd_mcp.go`
- **状态**：已完成（2026-08-12）
- **总结文件**：`docs/record/TF-051-knowledge-MCP工具与CLI-成功.md`

### TF-052 知识库前端（P2，依赖 TF-050、TF-023）

- **涉及模块**：`app/src/features/knowledge`、`app/src/features/tasks`、`app/src/features/imports`、`app/src/features/settings`、`internal/api`、`internal/config`
- **描述**：任务详情「资料」区（关联列表 + 阅读/编辑对话框 + 移除 + 添加搜索/路径，外部文件弹拷贝确认）；知识库页 `/project/:id/knowledge`（库列表 CRUD + 文档列表含 missing 徽标 + 检索视图命中片段 + 阅读/编辑/relink + 扫描按钮）；草稿确认流展示 `knowledge_files`（确认时自动关联）；**设置页配置表单**：全局「知识库」tab（knowledge 配置 + llm.embedding + QA-K23 向量开关置灰）+ 项目级 `default_doc_dir`；后端配置链路补齐（PUT /api/config、/api/project-config 支持 knowledge/embedding 节）；DTO 与后端字段级同步。
- **验收标准**：
  - [x] 组件测试：资料区、知识库页、检索结果、relink 对话框、草稿 knowledge_files 展示流
  - [x] 设置页测试：全局知识库 tab（含未配置模型置灰）、项目 default_doc_dir 保存
  - [x] `pnpm typecheck` / `pnpm lint` / `pnpm test` 全绿；`app/src/types/` 与后端 DTO 对齐；后端 knowledge 覆盖率 ≥ 90%
- **产出文件**：`app/src/features/knowledge/*`（3 组件 + 2 test）、`app/src/features/settings/KnowledgeSection.tsx`(+test)、`app/src/hooks/useKnowledge.ts`、`app/src/hooks/useConfig.ts`、`app/src/types/knowledge.ts`、`app/src/types/models.ts`、`app/src/features/tasks/TaskDetail.tsx`（资料区）、`app/src/features/imports/DraftReview.tsx`、`internal/api/handlers_config.go`、`internal/api/handlers_project_config.go`、`internal/config/validate.go`、`internal/config/project.go`
- **状态**：已完成（2026-08-13）
- **总结文件**：`docs/record/TF-052-知识库前端-成功.md`
...
## TF-055 守护进程空闲时重启（P3，独立）

- **涉及模块**：`cmd/daemon`、`internal/api`、`internal/version`、`app/electron`、`Makefile`、`.github/workflows/release.yml`
- **描述**：APP 启动检测 daemon 版本与自身不匹配 → 请求 daemon「结束当前任务后重启」：`GET /api/daemon/version`（免鉴权探测）→ `POST /api/daemon/restart`（仅 UI）→ `http.Server.Shutdown` 等待进行中请求完成（不打断 CLI 调用）→ 用 APP 提供的 bin_path 自我重生（setsid / CREATE_NEW_PROCESS_GROUP 跨平台）→ 旧进程退出。版本经 Makefile/release.yml 从 `app/package.json` 注入 `internal/version.Version`（`--version` 支持）；Electron `ensureDaemonRunning` 仅打包版启用比对（dev 不触发）。
- **验收标准**：
  - [x] 后端单测：version 端点、restart 权限（agent 403）、UI 接受 + 回调、bin_path 校验、幂等
  - [x] 端到端：临时 daemon 触发 restart → PID 变化、旧进程退出、版本一致；三平台交叉编译通过
  - [x] 前端：版本比对纯函数单测；typecheck/lint/test/build 全绿
- **状态**：已完成（2026-08-13）
- **总结文件**：`docs/record/TF-055-守护进程空闲时重启-成功.md`
