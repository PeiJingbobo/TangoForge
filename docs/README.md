...
├── FEATURES/               # 🧩 重点功能设计与实现方案
│   └── pert-graph.md       #   PERT 任务依赖图 v12（Graphviz/自然样条/无限画布）
├── task/                   # 📋 开发任务计划（三件套）
│   ├── MASTER-PLAN.md      #   主行动计划：阶段/里程碑/执行规则/完成定义
│   ├── TASKS.md            #   可执行任务清单：TF-001~TF-052 逐条细节
...
| `FEATURES/pert-graph.md` | v12.0                | PERT 任务依赖图重构方案（Graphviz 交叉最小化、自然样条平滑、搜索标记、无限画布）                                | PERT 布局/交互变化           |
| `BUILD-RELEASE.md`       | v1.0                 | 预发布版本编译指南（mac/Windows 双平台 electron-builder 打包）                                                      | 打包配置/流程变化            |
| `CI-CD-UPDATER.md`       | v1.0（已评审通过）   | CI/CD 自动打包发布 + 在线更新技术方案（macOS 未签名 dmg 手动安装 / Windows 自签名 + electron-updater；落地 TF-036） | 方案变更时                   |
| `KNOWLEDGE-BASE.md`      | v1.0（两轮 QA 通过） | 知识库功能需求与技术实施方案（文档引用注册表 + 任务关联 + 摘要/向量索引；落地 TF-044~TF-052）                       | 方案变更时                   |
...
| `task/TASKS.md`       | 可执行任务清单：TF-001 ~ TF-052（状态唯一事实源）          | 每任务完成时      |
| `task/OVERVIEW.md`    | 任务全景预览：总览表 / 泳道 / 依赖图 / 里程碑卡片          | 每任务完成时      |
| `record/`             | 任务总结归档（`TF-XXX-标题-结果.md`）                      | 每正式任务完成时  |
| `log/`                | 任务日志（`TF-XXX-标题.md`）                               | 任务进行中        |
| `troubleshooting/`    | 问题排查笔记（按类型分组：CI/打包/CLI/环境，含根因与解决） | 排查解决后追加    |

### 4.3 文件索引（按字母序）

- `AGENTS.md` — 开发约束与工作流（同步副本）
- `BUILD-RELEASE.md` — 预发布版本编译指南（本地手工打包）
- `CI-CD-UPDATER.md` — CI/CD 自动打包发布 + 在线更新技术方案（v1.0 已评审通过，TF-036 落地）
- `KNOWLEDGE-BASE.md` — 知识库功能需求与技术实施方案（v1.0，两轮 QA 通过，TF-044~TF-052 落地）
- `README.md` — 本文件
...
- `FEATURES/pert-graph.md` — PERT 任务依赖图 v12 实现方案
- `log/README.md` — 日志目录规范
- `log/TF-055-PERT任务图重构.md` — PERT v12 重构过程与验证记录
- `troubleshooting/2026-08-08.md` — 问题排查笔记（CI/打包/CLI/环境，含提交清单）
- `record/README.md` — 总结目录规范
- `record/TF-055-PERT任务图重构-成功.md` — PERT v12 优化完成总结
- `record/TF-055-PERT任务图重构-人工验证手册.md` — PERT v12 真实 App 人工验收步骤
