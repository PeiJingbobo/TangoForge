...
- **ARGUS 层级任务全景图（TF-028 / TF-055）**：父子关系与依赖关系共同参与 PERT 的 DAG 分层和路由，纯层级项目不再渲染为空白。
- 超大图首次进入时锚定分支最多的主根任务，避免可视区域落在无节点的整图中心。
- 同一对任务同时具有父子与依赖关系时只绘制一条连接线，并优先保留依赖语义。

### 变更

- `app/package.json` version 0.7.2 → 0.7.3。

### 验证

- 使用 ARGUS 规模的 122 子节点层级图补充布局回归测试。
- 前端 49 个测试文件、325 个测试通过；typecheck、lint、format、production build 通过。

## [0.7.2] - 2026-08-18

### 新增

- **PERT 全景图 v12（TF-028 / TF-055）**：圆形任务节点、Graphviz 交叉最小化、自然三次样条平滑路径、节点避障与疏散布局、根谱系配色、连接线选择及上下游高亮、搜索结果同心圆与自动聚焦、无限画布和缩放箭头。
- 全景图保留力导向视图并支持视图偏好记忆；画布空白使用抓手指针，连接线命中区域使用普通箭头。
- 导入草稿识别文档原始状态，确认时并入项目状态机；优先级空字符串按未指定处理。

### 变更

- `app/package.json` version 0.7.0 → 0.7.2。
- Skills 管理页与引导页共用中英文 AGENTS 提示词，补齐知识库工具和任务状态同步规则。
- 导入类 LLM 请求允许覆盖默认超时，并区分请求超时与守护进程断连。

### 修复

- Knowledge API 参数与 Query 缓存刷新对齐；兼容无 `knowledge.` 前缀的后端 WebSocket 事件。
- 修复知识库页面和任务数据在索引队列更新后未及时刷新的问题。

### 验证

- 前端 49 个测试文件、322 个测试通过；typecheck、lint、format、production build 通过。
- Go `vet` 与普通全量测试通过；`internal/task` 覆盖率 91.4%。本机覆盖率汇总受 Go 工具链缺少 `covdata` 影响，标签发布流水线将在 GitHub Actions 标准 Go 1.25 环境复验。
